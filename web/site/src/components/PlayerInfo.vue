...
const playerInfo = computed(() => {
    const player = props.playerNumber == 1 ? store.player1 : store.player2;
    if (player == null) {
...
<template>
    <div class="flex flex-row gap-2 relative z-10">
        ...
        <div v-if="dropdownVisible" class="absolute flex flex-col left-[60px] top-[22px] bg-container/95 w-[220px] p-2 rounded-[8px]">
            <div @click="switchPlayer(player)" v-for="player in store.availablePlayers" class="flex flex-row gap-2 items-center group cursor-pointer min-h-[30px]">
