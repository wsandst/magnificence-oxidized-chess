...
impl Board {
    fn extract_bishop_moves(&self, moves : &mut Vec<Move>, bishop_occupancy: u64, same_color_occupancy: u64, opposite_occupancy: u64) {
        let occupancy = same_color_occupancy | opposite_occupancy;
        let mut occupancy_mask = bishop_occupancy;
    ...
    pub(in crate::core) fn generate_white_bishop_moves(&self, moves : &mut Vec<Move>, state: &MovegenState) {
        let bishop_occupancy = self.piece_sets[Piece::WhiteBishop.to_u8() as usize];
        self.extract_bishop_moves(moves, bishop_occupancy, state.white_occupancy, state.black_occupancy);
    }

    pub(in crate::core) fn generate_black_bishop_moves(&self, moves : &mut Vec<Move>, state: &MovegenState) {
        let bishop_occupancy = self.piece_sets[Piece::BlackBishop.to_u8() as usize];
        self.extract_bishop_moves(moves, bishop_occupancy, state.white_occupancy, state.black_occupancy);
    }
}
...
mod tests {
    ...
    fn test_knight_move_gen() {
        ...
        board.generate_white_bishop_moves(&mut moves, &movegen_state);
