...
impl Board {
    ...
    pub(in crate::core) fn generate_white_king_moves(&self, moves : &mut MoveList, state: &MovegenState) {
        ...
        if state.only_captures {
            self.extract_king_moves(moves, white_king_index, state.white_occupancy, state.threatened_squares | !state.black_occupancy);
        } else {
    ...
    pub(in crate::core) fn generate_black_king_moves(&self, moves : &mut MoveList, state: &MovegenState) {
        ...
        if state.only_captures {
            self.extract_king_moves(moves, black_king_index, state.black_occupancy, state.threatened_squares | !state.white_occupancy);
        } else {
